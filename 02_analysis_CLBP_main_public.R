...
# =====================================================############################################################
# 02_analysis_CLBP_main.R
############################################################

# Workspace leeren (wichtig für reproduzierbare Analyse)
rm(list = ls())

############################################################
# Analyse des AU-Datensatzes (Resolve Study):
# Analyse: FreBAQ & TPD -> NRS (AU + CH)
############################################################

## 0. Pakete laden ----
# install.packages("tidyverse")   # einmalig ausführen
# install.packages("Hmisc")       # falls noch nicht vorhanden
# install.packages("car")         # für VIF
...
library(Hmisc)
library(car)
# Ordnerstruktur für Outputs sicherstellen
...
# Data are not included in this public repository due to ethical and data governance restrictions.
ch_raw <- readr::read_csv("path/to/swiss_cohort_dataset.csv") %>%
  janitor::clean_names()
# Inspect structure
...
# --------------------------------------------------
# Logbook / Codebook laden, um Variablen zu identifizieren
# --------------------------------------------------

path_au_logbook <- "/Volumes/shared.zhaw.ch/G-PT-Resolve-Swiss-normal/z_DATA_AUS_from_Matt/FROM_MATT_resolve_full clean_logbook.xlsx"
